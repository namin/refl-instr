...
(define (show-instr d pre i)
  (let ((r (assq (car i) pre)))
    (when r
      ((cdr r) d i)))
  (cond ((eq? (car i) 'number)
        ...
        ((eq? (car i) 'quote)
         `(quote ,(cadr i)))
        ((eq? (car i) 'if-then)
         `(if-then ,(show-val (cadr i)) ,(show-instr d pre (caddr i))
                   ,(show-val (cadddr i)) ,(show-instr d pre (caddddr i))))
        ((eq? (car i) 'if-else-empty)
         `(if-else-empty ,(show-val (cadr i)) ,(show-instr d pre (caddr i))))
        ((eq? (car i) 'if-else)
         `(if-else ,(show-val (cadr i)) ,(show-instr d pre (caddr i))
                   ,(show-val (cadddr i)) ,(show-instr d pre (caddddr i))))
        ((eq? (car i) 'define)
         `(define ,(cadr i) ,(show-val (caddr i)) ,(show-instr d pre (cadddr i))))
        ((eq? (car i) 'set!)
         `(set! ,(cadr i) ,(show-val (caddr i)) ,(show-instr d pre (cadddr i))))
        ((eq? (car i) 'lambda)
         `(lambda ,(cadr i) ,(caddr i)))
        ((eq? (car i) 'begin)
         `(begin ,(show-val (cadr i)) ,(map (lambda (xi) (show-instr d pre xi)) (caddr i))))
        ((eq? (car i) 'let)
         `(let ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i)) ,(show-instr (1+ d) pre (caddddr i))))
        ((eq? (car i) 'nil)
         `(nil))
        ((eq? (car i) 'cons)
         `(cons (,(show-val (caadr i)) ,(show-instr d pre (cadadr i)))
                (,(show-val (caaddr i)) ,(show-instr d pre (cadaddr i)))))
        ((eq? (car i) 'app)
         `(app ,(show-val (cadr i)) ,(show-instr d pre (caddr i))
               ,(show-val (cadddr i)) ,(show-instr d pre (caddddr i))))
        ((eq? (car i) 'apply-proc)
         `(apply-proc ,(show-val (cadr i)) ,(show-val (caddr i)) ,(show-val (cadddr i))))
        ((eq? (car i) 'lambda-body)
         `(lambda-body ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i)) ,(show-instr (1+ d) pre (caddddr i))))
        (else (error 'show-instr d pre (list 'unknown i)))))

(define (indent n)
  (string->symbol (make-string (1+ n) #\*)))

(define show-instr-pre (list
  (cons 'var (lambda (d i) (let ((v (caddr i)))
                        (unless (or (and (pair? v) (eq? (car v) lambda-tag))
                                    (procedure? v))
                          (pretty-print `(,(indent d) var ,(cadr i) ,(show-val (caddr i)))))) ))
  (cons 'set! (lambda (d i) (pretty-print `(,(indent d) set! ,(cadr i) ,(show-val (caddr i))))))
  (cons 'lambda-body (lambda (d i) (pretty-print `(,(indent d) lambda... ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i))))))))
    ...
    (if (eq? r 'exit) 'exit
        (base-eval
         ...
         (lambda (r ri)
           (display r)
           (newline)
           (show-instr 0 show-instr-pre ri)
           (newline)
           (repl-inner env))))))
