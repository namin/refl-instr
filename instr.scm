...
(define (my-instr i)
  (cond ((eq? (car i) 'number)
        ...
        ((eq? (car i) 'quote)
         `(quote ,(cadr i)))
        ((eq? (car i) 'if-then)
         `(if-then ,(show-val (cadr i)) ,(my-instr (caddr i))
                   ,(show-val (cadddr i)) ,(my-instr (caddddr i))))
        ((eq? (car i) 'if-else-empty)
         `(if-else-empty ,(show-val (cadr i)) ,(my-instr (caddr i))))
        ((eq? (car i) 'if-else)
         `(if-else ,(show-val (cadr i)) ,(my-instr (caddr i))
                   ,(show-val (cadddr i)) ,(my-instr (caddddr i))))
        ((eq? (car i) 'define)
         `(define ,(cadr i) ,(show-val (caddr i)) ,(my-instr (cadddr i))))
        ((eq? (car i) 'lambda)
         `(lambda ,(cadr i) ,(caddr i)))
        ((eq? (car i) 'nil)
         `(nil))
        ((eq? (car i) 'cons)
         `(cons (,(show-val (caadr i)) ,(my-instr (cadadr i)))
                (,(show-val (caaddr i)) ,(my-instr (cadaddr i)))))
        ((eq? (car i) 'app)
         `(app ,(show-val (cadr i)) ,(my-instr (caddr i))
               ,(show-val (cadddr i)) ,(my-instr (caddddr i))))
        ((eq? (car i) 'apply-proc)
         `(apply-proc ,(show-val (cadr i)) ,(show-val (caddr i)) ,(show-val (cadddr i))))
        ((eq? (car i) 'lambda-body)
         (pretty-print `(lambda... ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i))))
         `(lambda-body ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i)) ,(my-instr (caddddr i))))
        (else (error 'my-instr (list 'unknown i)))))
