...
(define (show-instr pre i)
  ...
  (cond ((eq? (car i) 'number)
        ...
        ((eq? (car i) 'quote)
         `(quote ,(cadr i)))
        ((eq? (car i) 'if-then)
         `(if-then ,(show-val (cadr i)) ,(show-instr pre (caddr i))
                   ,(show-val (cadddr i)) ,(show-instr pre (caddddr i))))
        ((eq? (car i) 'if-else-empty)
         `(if-else-empty ,(show-val (cadr i)) ,(show-instr pre (caddr i))))
        ((eq? (car i) 'if-else)
         `(if-else ,(show-val (cadr i)) ,(show-instr pre (caddr i))
                   ,(show-val (cadddr i)) ,(show-instr pre (caddddr i))))
        ((eq? (car i) 'define)
         `(define ,(cadr i) ,(show-val (caddr i)) ,(show-instr pre (cadddr i))))
        ((eq? (car i) 'set!)
        ...
        ((eq? (car i) 'nil)
         `(nil))
        ((eq? (car i) 'cons)
         `(cons (,(show-val (caadr i)) ,(show-instr pre (cadadr i)))
                (,(show-val (caaddr i)) ,(show-instr pre (cadaddr i)))))
        ((eq? (car i) 'app)
         `(app ,(show-val (cadr i)) ,(show-instr pre (caddr i))
               ,(show-val (cadddr i)) ,(show-instr pre (caddddr i))))
        ((eq? (car i) 'apply-proc)
         `(apply-proc ,(show-val (cadr i)) ,(show-val (caddr i)) ,(show-val (cadddr i))))
        ((eq? (car i) 'lambda-body)
         `(lambda-body ,(cadr i) ,(show-val (caddr i)) ,(show-val (cadddr i)) ,(show-instr pre (caddddr i))))
